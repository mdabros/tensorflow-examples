...
def transfer_learn_model_fn(features, labels, mode):
    ...
    # Use features from pre-trained model/module
    module_features = module(input_layer)
   
    # Dropout layer
    dropout = tf.layers.dropout(module_features, rate=0.4, training=mode == tf.estimator.ModeKeys.TRAIN)
...
def main(unused_argv):
    with tf.Graph().as_default() as g:       
        ...
        model_directory = os.getcwd() + "/transfer-learn-model/"
        if os.path.exists(model_directory):
            os.remove(model_directory)

        # Create an estimator
        classifier = tf.estimator.Estimator(
            model_fn=transfer_learn_model_fn, model_dir=model_directory)
